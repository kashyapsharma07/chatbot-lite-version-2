...
│   └── logs/               # chat.log (user chats), server.log (startup noise)
...
Chat logs are kept separate from server noise so they stay easy to review.

| File | Purpose | Rotation |
|------|---------|----------|
| `backend/logs/chat.log` | User queries + bot responses (JSON lines) | 2 MB/file, keeps 10 files (~20 MB total) |
| `backend/logs/server.log` | Startup, API, and Flask noise | 1 MB/file, keeps 3 files |
...
- `query`
- `response`
- `retrieval_s` / `generation_s`

**View recent chats:**

```bash
cd backend
python view_chat_logs.py --last 20
python view_chat_logs.py --session <session-id>
```

**Disable logging** (optional): set `ENABLE_LOGGING=OFF` in `.env`.

The old `queries.log` mixed chat data with HuggingFace/Flask output. You can archive or delete it once you no longer need the history.
...
- `ckpcmcdataset.json` - Main knowledge base documents
